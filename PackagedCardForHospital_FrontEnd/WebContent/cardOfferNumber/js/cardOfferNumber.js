...
function Operation() {
    ...
    this.FindCountStuTable = function (Conditions) {
        var url = "../countArrIDTable";
        var msg = initOperation.ajaxGetJSON(url, Conditions);
        var obj = JSON.parse(msg);
    ...
    this.AddTableLine = function (obj) {
        ...
        for (var i = 0; i < obj.length; i++) {
            $("#appointTable").append("<tr class = 'childTR' id =" + obj[i].id + " >" +
                ...
                // "<input onclick = 'initOperation.modifyStuData(this)' type = 'button' value = '修改' >" +
                // "</td > " +
                "</tr>");
        }
