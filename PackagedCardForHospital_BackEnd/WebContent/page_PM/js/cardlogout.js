...
function data() {
    ...
    this.makeRefreshJSON = function () {
        var tempJSON = {
            "CNUMStart": PageData.CNUMStart = $("#CNUMStart").val(),
            "CNUMEND": PageData.CNUMEND = $("#CNUMEND").val(),
            "CSatus": PageData.CSatus = $("#CSatus").val(),
            "CurrentPage": PageData.CurrentPage,
            "Limit": PageData.Limit
        };
        return tempJSON;
...
function Operation() {
    ...
    this.RefreshPageAndTable = function () {
        ...
        OperationInit.RefreshTable(PageData.makeRefreshJSON());
        OperationInit.RefreshPage(PageData.makeRefreshJSON());
    };
    ...
    this.AddTableLine = function (obj) {
        ...
        for (var i = 0; i < obj.length; i++) {
            // if (obj[i].card_STATUS == -1) continue;
            var btn = obj[i].card_STATUS == 1 ? "<input onclick = 'OperationInit.delStuData(this)' type = 'button' value = '注销' >" : "";
            $("#table").append("<tr id =" + obj[i].id + " >" +
                "<td><span>" + obj[i].id + "</span></td>" +
                "<td><span>" + obj[i].card_NUM + "</span></td>" +
                "<td><span>" + (obj[i].card_STATUS == 1 ? "待出售" : obj[i].card_STATUS == -1 ? "已注销" : "已出售") + "</span></td>" +
                "<td>" +
                btn +
                "</td > " +
                "</tr>");
    ...
    this.delStuData = function (event) {
        ...
        console.log($(event.parentElement.parentElement.children[2].children[0]).html());
        var status = $(event.parentElement.parentElement.children[2].children[0]).html();
        if (status != "已出售") {
