...
function data() {
    ...
    this.makeTempJSON = function () {
        var tempJSON = {
            "CNUM": PageData.CNUM = $("#CNUM").val(),
            "CSatus": PageData.CSatus = $("#CSatus").val(),
            "requestor": PageData.requestor = $("#requestor").val(),
            "CurrentPage": PageData.CurrentPage,
            "Limit": PageData.Limit
        };
        return tempJSON;
    ...
    this.AddTableLine = function (obj) {
        ...
        for (var i = 0; i < obj.length; i++) {
            $("#table").append("<tr id =" + obj[i].id + " >" +
                ...
                "<td><span>" + (obj[i].card_STATUS == 1 ? "待出售" : obj[i].card_STATUS == -1 ? "已注销" : "") + "</span></td>" +
                "<td><span>" + obj[i].card_REQUESTOR + "</span></td>" +
                "<td><span>" + (obj[i].card_USER == null ? "" : obj[i].card_USER) + "</span></td>" +
                "<td>" +
    ...
    this.getRequestorInf = function () {
        $.ajax({
            ...
            success: function (msg) {
                ...
                var obj = JSON.parse(msg);
                for (var i = 0; i < obj.length; i++) {
                    $("#requestor").append("<option value='" + obj[i].id + "'>" + obj[i].name + "</option>>")
                }
            }
        })
    };
