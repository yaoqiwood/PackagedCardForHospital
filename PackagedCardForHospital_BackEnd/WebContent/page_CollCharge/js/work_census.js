...
function Operation() {
    ...
    this.AddTableLine = function (obj) {
        ...
        for (var i = 0; i < obj.length; i++) {
            $("#table").append("<tr id =" + obj[i].b_EXCUTORID + " >" +
                "<td><span>" + obj[i].b_EXCUTORID + "</span></td>" +
                "<td><span>" + obj[i].sold_CARD + "</span></td>" +
                "<td><span>" + (obj[i].change_CARD) + "</span></td>" +
                "<td><span>" + obj[i].exid_CARD + "</span></td>" +
                "</tr>");
        }
    };
    ...
    this.fillWorkman = function () {  
        $.ajax({
            ...
            success: function (response) {
                ...
                for (var i = 0; i < obj.length; i++){
                    $("#workman").append("<option value = '"+obj[i].id+"'>"+obj[i].name+"</option>");
                }
            }
        });
    }
